namespace Supercell.Laser.Server.Networking.Session
{
    ...
    public static class Sessions
    {
        ...
        public static int Count
        {
            get
            {
                return ActiveSessions.Values.Count(session => session.Home.Avatar.NameSetByUser); //brawler olanları sayma
            }
        }
        ...
        public static void StartShutdownDc()
        {
            Maintenance = true;
            foreach (var session in ActiveSessions.Values.ToArray())
            {
        ...
        public static void Debug()
        {
            ...
            foreach (var session in ActiveSessions.Values.ToArray())
            {
                session.Connection.Send(new DebugMenuMessage());
                session.Connection.Avatar.BattleStartTime = new DateTime();
            }
