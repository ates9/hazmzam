    ...
    public static class Alliances
    {
        ...
        public static List<Alliance> GetRankingList()
        {
            ...
            try
            {
                ...
                list = list.OrderByDescending(alliance => alliance.OnlinePlayers).ToList();
                return list;
            }
            catch (Exception exception)
            {
        ...
        public static List<Alliance> GetRankingLista()
        {
            #region GetGlobal
        ...
        public static List<Alliance> GetRankingListSearch(string searchValue)
        {
            Console.WriteLine("arama yapıldı:" + searchValue);
            var list = new List<Alliance>();
            ...
            try
            {
                using (var connection = new MySqlConnection(ConnectionString))
                {
                    ...
                    using (var cmd = new MySqlCommand($"SELECT * FROM alliances WHERE `Name` LIKE @searchValue ORDER BY `Trophies` DESC LIMIT 200",
                        connection))
                    {
                ...
                Console.WriteLine("liste:" + list + " liste sayı:" + list.Count);
                return list;
            }
            catch (Exception exception)
            {
        ...
        public static List<Alliance> GetRankingListSearchHashtag(string searchValue)
        {
            Console.WriteLine("arama yapıldı:" + searchValue);
            var list = new List<Alliance>();
            ...
            try
            {
                using (var connection = new MySqlConnection(ConnectionString))
                {
                    ...
                    using (var cmd = new MySqlCommand($"SELECT * FROM alliances WHERE `Id` = @searchValue ORDER BY `Trophies` DESC LIMIT 200",
                        connection))
                    {
                        long asd = LogicLongCodeGenerator.ToId(searchValue);
                        cmd.Parameters.AddWithValue("@searchValue",  asd );
